...
module.exports = class PrometheusMetricsConsumer extends Writable {
	constructor({ bucketStepFactor = 1.15, bucketStepCount = 45, bucketStepStart = 0.001, logger, client } = {}) {
		...
		if (!is.validPromClient(client))
			throw new Error('Provided value to argument "client" is not a Prometheus client');
		if (!is.validBucketStepFactor(bucketStepFactor))
			throw new Error('Provided value to argument "bucketStepFactor" is not legal');
		if (!is.validBucketStepCount(bucketStepCount))
			throw new Error('Provided value to argument "bucketStepCount" is not legal');
		if (!is.validBucketStepStart(bucketStepStart))
			throw new Error('Provided value to argument "bucketStepStart" is not legal');
	...
	override(name = "", config = {}) {
		...
		if (!is.validBucketStepFactor(bucket.bucketStepFactor))
			throw new Error('Provided value to argument "config.bucket.bucketStepFactor" is not legal');
		if (!is.validBucketStepCount(bucket.bucketStepCount))
			throw new Error('Provided value to argument "config.bucket.bucketStepCount" is not legal');
		if (!is.validBucketStepStart(bucket.bucketStepStart))
			throw new Error('Provided value to argument "config.bucket.bucketStepStart" is not legal');
	...
	_write(metric, enc, cb) {
		if (this.typeFor(metric) === metricTypes.HISTOGRAM) {
			try {
				...
				this.logger.error(
					err,
					`failed to generate prometheus histogram for metric "${JSON.stringify(metric)}"`,
				);
			}
			...
			try {
				...
				this.logger.error(err, `failed to generate prometheus gauge for metric "${JSON.stringify(metric)}"`);
			}
		} else if (this.typeFor(metric) === metricTypes.SUMMARY) {
			try {
				this.summary(metric);
			} catch (err) {
				this.logger.error(err, `failed to generate prometheus summary metric for "${JSON.stringify(metric)}"`);
			}
		} else {
			try {
				this.counter(metric);
			} catch (err) {
				this.logger.error(err, `failed to generate prometheus counter for metric "${JSON.stringify(metric)}"`);
			}
		}
		cb();
	}
