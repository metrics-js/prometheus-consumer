...
function createBuckets({ bucketFunction, bucketStepStart, bucketStepFactor, bucketStepCount }) {
	return bucketFunction(bucketStepStart, bucketStepFactor, bucketStepCount).map((f) => Math.round(f * 10000) / 10000);
}
...
function createHistogram(Histogram, bucketFunction, metric, options) {
	...
	const overrides = meta && Array.isArray(meta.buckets) && meta.buckets.length && meta.buckets;
